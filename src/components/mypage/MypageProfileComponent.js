...
const MypageProfileComponent = () => {
    ...
    window.onpopstate = function(event) {
        window.history.pushState(null, '', window.location.replace('../../mypage')); // http://localhost:3000/mypage/profile/confirm
    };
    ...
    const [profileImage, setProfileImage] = useState(memberImage ? `http://15.165.219.211:8080/api/productreview/view/${memberImage}` : "/images/profile.png")
    ...
    useEffect(() => {
        if (memberImage) {
            setProfileImage(`http://15.165.219.211:8080/api/productreview/view/${memberImage}`);
        } else {
    ...
    const clickModify = async()=>{
        ...
        try {
            ...
            if (modifyMemberPw) {
                // 비밀번호 글자수 제한
                if(modifyMemberPw.length < 4 || modifyMemberPw.length > 20){
