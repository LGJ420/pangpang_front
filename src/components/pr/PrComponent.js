        ...
        if (entry.isIntersecting) {
          ...
          titleRefs.current.forEach((titleRef, index) => {
            if (entry.target === titleRef) {
              entry.target.classList.add(styles.prTitleAnimation);
            }
          });
          contentRefs.current.forEach((contentRef, index) => {
            if (entry.target === contentRef) {
              entry.target.classList.add(styles.prContentAnimation);
            }
          });
        }
      });
      ...
      {sections.map((section, index) => (
        <section key={index} className={styles.section}>
          <div className='w-[1350px] m-auto flex items-center'>
            <div className='w-1/2'>
              <img ref={el => imageRefs.current[index] = el} className={`${styles.prImage} w-[50rem] h-[35rem] object-cover rounded-2xl`} src={section.image} />
            </div>
            <div className='w-1/2 p-3'>
              <h3 ref={el => titleRefs.current[index] = el} className={`${styles.prTitle} text-5xl font-semibold font-poor-story p-7`}>
                {section.title}
              </h3>
              <p ref={el => contentRefs.current[index] = el} className={`${styles.prContent} px-7`}>
                {section.content}
              </p>
            </div>
          </div>
